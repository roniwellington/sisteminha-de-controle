...
function createTask(tarefa) {
    ...
    button.addEventListener('click', (event) => {
        ...
        selecionarTarefaParaEditar(tarefa, paragraph)
    })
    ...
    svgIcon.addEventListener('click', (event) => {
        if (tarefa == tarefasSelecionada) {
            event.stopPropagation()
            button.setAttribute('disabled', true)
            li.classList.add('app__section-task-list-item-complete')
            tarefasSelecionada.concluida = true
            updateLocalStorage()
        }
...
btnDeletar.addEventListener('click', () => {
    if(tarefasSelecionada) {
        const index = tarefas.indexOf(tarefasSelecionada)
        ...
        tarefas.filter(t=> t!= tarefasSelecionada)
        itemTarefaSelecionada = null
        tarefasSelecionada = null
    }
    updateLocalStorage()
...
toggleFormTaskBtn.addEventListener('click', () => {
    formLabel.textContent = 'Adicinando tarefa'
